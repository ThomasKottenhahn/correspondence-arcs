...
mod test{
    ...
    fn building_in_gate(){
        ...
        let _ = actions::execute_action(&game_state, data::Action::Build{target_system, build_type});
    }
    ...
    fn build_in_full_system(){
        ...
        let g1 = actions::execute_action(&game_state, data::Action::Build{target_system, build_type: build_type.clone()});
        let g2 = actions::execute_action(&g1, data::Action::Build{target_system, build_type: build_type.clone()});
        let _ = actions::execute_action(&g2, data::Action::Build{target_system, build_type: build_type.clone()});
    }

    #[test]
    #[should_panic(expected = "System is unused")]
    fn building_in_unused_system(){
        let test_setup: data::SetupCard = data::SetupCard{players: 2,cluster_out_of_play: vec![3]};
        let game_state: data::GameState = board::setup_game(&test_setup);
