...
mod test{
    ...
    fn test_building_ships(){
        let test_setup: data::SetupCard = data::SetupCard { players: 2, cluster_out_of_play: vec![0,5], a_locations: vec![20,12], b_locations: vec![17, 18], c_loactions: vec![2,13,4,14]};
    ...
    fn test_building_city(){
        let test_setup: data::SetupCard = data::SetupCard { players: 2, cluster_out_of_play: vec![0,5], a_locations: vec![20,11], b_locations: vec![17, 18], c_loactions: vec![2,13,4,14]};
        let game_state: data::GameState = board::setup_game(&test_setup);
